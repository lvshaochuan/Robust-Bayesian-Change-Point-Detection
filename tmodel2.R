...
FFBS.t <- function(Uniformtimes, JumpProb, eventtimes, obs, param, DF, Sigma, Ui){
n <- length(obs)
...
# Alphas[i] <- rinvchisq(1, df=N[i], scale= mean(temp0[cp[i] : (cp[i+1] - 1)]))
mu[i] <- rnorm(1, temp2/temp1, 1/temp1)
Sigma2[i] <- rgamma(1, shape = N[i]*DF/2, rate = DF*temp1/2)

for (j in cp[i] : (cp[i+1] - 1)){
Ui[j] <- rinvchisq(1, df=DF + 1, scale=(DF*Sigma2[i] + (obs[j] - mu[i])^2)/(DF + 1))
#temp0[j] <- (obs[j] - mu[i])^2/Ui[j]
}
#Taus[i] <- rgamma(1, shape = N[i]*DF/2, rate = DF*temp1/2)
...
mu[m+1] <- rnorm(1, temp2/temp1, 1/temp1)
Sigma2[m+1] <- rgamma(1, shape = N[m+1]*DF/2, rate = DF*temp1/2)

for (j in cp[m+1] : n){
Ui[j] <- rinvchisq(1, df=DF + 1, scale=(DF*Sigma2[m+1] + (obs[j] - mu[m+1])^2)/(DF + 1))
#temp0[j] <- (obs[j] - mu[m+1])^2/Ui[j]
}
#Taus[m+1] <- rgamma(1, shape = N[m+1]*DF/2, rate = DF*temp1/2)
#Alphas[m+1] <- rinvchisq(1, df=N[m+1], scale= mean(temp0[cp[m+1] : n]))

} else{
       cp <- NULL
       Q <- NULL
#      theta <- rnorm(1, mean(obs), sd=Sigma/n)
       mu <- rnorm(1, sum(obs/Ui)/sum(1/Ui), 1/sum(1/Ui))
       Sigma2 <- rgamma(1, shape = n*DF/2, rate = DF*sum(1/Ui)/2)
       for (k in 1:n) Ui <- rinvchisq(1, df=DF + 1, scale=(DF*Sigma2 + (obs[k] - mu)^2)/(DF + 1))
#       temp0 <- (obs-mu)^2/Ui
#       Taus <- rgamma(1, shape = n*DF/2, rate = DF*sum(1/Ui)/2)
#       Alphas <- rinvchisq(1, df=n, scale= mean(temp0))
      }
...
return(list(Number=m+1, cp=cp, CP=CP, Q=Q, states= S, filtering=filtering, param=mu, Sigma2=Sigma2, Ui=Ui)) 
